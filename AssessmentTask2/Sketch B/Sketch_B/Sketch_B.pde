...
//changed background to move with mouse. 
  background(mouseY, mouseX, 30);
  smooth();
  ...
  for (int gridY=0; gridY<tileCount; gridY++) {
    for (int gridX=0; gridX<tileCount; gridX++) {
      ...
      //reversed movement so that all are now moving away from mouse.
      if (fillMode == 2) {
        currentShape.disableStyle();
        float a = map(dist(mouseY,mouseX,posY,posX), 0,maxDist, 255,0);
        fill(shapeColor, a);      
      }
      if (fillMode == 3) {
        currentShape.disableStyle();
...
void keyPressed() {
  ...
  if (key == "8") currentShape = loadShape(

  if (keyCode == UP) shapeColor -=5;
